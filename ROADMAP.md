...
| 3 | **Edge runtime (NAS / Pi)** | Gateway runs on smaller hardware = more deployments. Already cross-compiled to `linux/arm64` in v0.4+; needs `.deb` / `.spk` polish for one-click NAS install. | Package GoReleaser artifacts as `.deb` / `.spk` | **S** | v0.7 |
...
| --- | --- | --- | --- | --- | --- |
| P1 | **Bedrock + Vertex egress adapters** | Two more vendor routes alongside the existing Anthropic + OpenAI fallback. Lets orgs with AWS / GCP spend keep using their existing billing path. | ✅ Detection + routing pipe + config shipped 2026-06-07 (`anthropic.*`/`amazon.*`/... → Bedrock, `gemini-*` → Vertex; new fields `router.fallback.bedrock_region` / `vertex_project` / `vertex_location`; env `FLOCK_BEDROCK_REGION` / `FLOCK_VERTEX_PROJECT`). **Remaining**: SigV4 signing (Bedrock) + ADC auth (Vertex) — currently return HTTP 501 with a clear setup hint. | **S** | v0.7 |
