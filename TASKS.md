...
- ~~**Automatic GGUF distribution from leader to workers**~~ — ✅ shipped 2026-06-07 (M5-T12, partial). For `source.type=file` entries, `CreateSharded` now HEADs each worker's `/v1/process/file` and streams the GGUF over `/v1/process/upload` when missing. Sha256-verified, no-op when already present. Auto-download from HF to the leader (the *other* half of M5-T12) is still open.
