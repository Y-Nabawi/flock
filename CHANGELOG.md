...
- **Router request hedging** — opt-in per-request fan-out. With `router.hedge_replicas: 2` in `config.yaml` (capped at 3), a client adding `flock.hedge: true` to the body (or `X-Flock-Hedge: 1` header) fires the chat request to the top-N least-loaded workers concurrently and returns whichever stream opens first; the losers' contexts are cancelled and their inflight gauge is decremented. Tail-latency win for ~2× the engine cost. Hedging skips retry / fallback / latency reorder — the operator already accepted the cost trade. Metric: `flock_router_hedge_total{outcome=win|cancelled|error}`. Falls back to the normal single-pick path when fewer than 2 eligible workers exist (cooldowns + stale heartbeats are excluded).
