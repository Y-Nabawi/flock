...
- **OpenAI-compatible hosted gateway adapters** — passthrough to OpenRouter, Groq, Together, and Fireworks. Slash-namespaced prefixes (`openrouter/<model>`, `groq/<model>`, etc.) are recognized by `Vendor()` and routed through a shared OpenAI-shape proxy that strips the prefix from the request body's `model` field before forwarding. Env: `OPENROUTER_API_KEY`, `GROQ_API_KEY`, `TOGETHER_API_KEY`, `FIREWORKS_API_KEY`. URLs default to each vendor's public endpoint; override per-vendor via `router.fallback.{openrouter,groq,together,fireworks}_url`.
